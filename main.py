...
spawn_delay = 2000  # start at 2 sec
...
def reset_game():
    ...
    global player, zombies, bullets, score, game_over, paused, spawn_delay
    player = Player(WIDTH//2, HEIGHT//2, 5, "./assets/player.png")
    zombies = []
    bullets = []
    score = 0
        ...
        if not game_over:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and not paused:
                    ...
                    bullets.append(Bullet(player.rect.centerx, player.rect.centery, angle, 12, "./assets/bullet.png"))
                    bullet_sound.play()
            ...
            if not paused:
                ...
                for bullet in bullets[:]:
                    bullet.move()
                    if bullet.is_expired(WIDTH, HEIGHT):
                    ...
                    for bullet in bullets[:]:
                        if zombie.rect.colliderect(bullet.rect):
                            ...
                            if zombie.health <= 0:
                                ...
                                # Difficulty scaling
                                if score % 10 == 0 and spawn_delay > 500:
                                    spawn_delay = max(500, spawn_delay - 200)
                                    pygame.time.set_timer(SPAWN_EVENT, spawn_delay)
                            break
            ...
            draw_button(pause_button, "Resume" if paused else "Pause", (0, 100, 200))
